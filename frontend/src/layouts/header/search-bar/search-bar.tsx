            ...
            <PopoverDescription>
              <div className="w-full flex flex-col">
                ...
                <div className="w-full flex flex-col gap-3">
                  ...
                  <div className="flex flex-col gap-2">
                      {result.posts.map(p => (
                        <SlimCard className="flex">
                          <div className="flex-1">
                          <p className="text-foreground text-xl ">{p.title}</p>
                          <p>{p.show.type}: {p.show.name}</p>
                          </div>
                          <div className="w-20">
                            <p className="flex gap-3 items-center text-sm"><ThumbsUp  className="w-4"/>{p.likes < 1000? p.likes: p.likes/1000 + "k"}</p>
                            <p className="flex gap-3 items-center" text-sm><MessageCircle className="w-4"/>{p.commentsCount}</p>
                          </div>
                        </SlimCard>
                      ))}
                  </div>
